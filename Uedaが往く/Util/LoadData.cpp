...
namespace
{
	...
	constexpr int kStatusNum = 17;		// 1キャラクターのステータス数
...
void LoadData::LoadCharaData(CharacterBase& data, int charType)
{
	...
	data.m_status.kickPower = m_charData[7 + charType * kStatusNum];
	data.m_status.maxMoveSpeed = m_charData[8 + charType * kStatusNum];
	data.m_status.fightWalkSpeed = m_charData[9 + charType * kStatusNum];
